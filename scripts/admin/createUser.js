...
import Role from '../../classes/user-role.js';
...
function createDropdown(e) {
    ...
    text.id = 'selection';
    text.textContent = 'Select your pronoun';
    button.appendChild(text); */

    // Nesting span inside button to get caret symbol.
...
function sendData() {
    ...
    if(gender === 'Custom') {
        ...
        if(customGender === 'Select your pronoun') {
            ...
            user.preferredPronoun = customGender;
        }
    }

    /* The two fields, gender and preferredPronoun are represented
    * as enums on the server, so the string has to be in all caps
    * for the framework on the backend to match the fields in the
    * Java class.
    */
    user.gender = user.gender.toUpperCase();

    if(user.preferredPronoun != null)
        user.preferredPronoun = user.preferredPronoun.toUpperCase();

    const requestBody = JSON.stringify(user);

    let request = new XMLHttpRequest();

    // Opening connection to backend Java API on local server with Get method.
    let URI = 'http://localhost:8080/bankingapp/api/admin';
    ...
    request.onload = () => {
        ...
        if(request.status === 201) {
            feedback.textContent = 'Account Was Successfully Created.';
            ...
            feedback.textContent = 'This Action Requires Higher Privileges';
            ...
            feedback.textContent = 'You Must Be Logged In To Perform This Action.';
            feedback.style.color = 'red';
            window.setTimeout(function() {
    ...
    request.onerror = () => {
        let feedback = document.getElementById('feedback');
        feedback.textContent = 'Error Creating Account!';
